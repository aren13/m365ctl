    ...
    if args.rules:
        ...
        try:
            if plan_out is None:
                ...
                emit_only = True
            # Build a GraphClient so the runner can lazy-fetch
            # ``internetMessageHeaders`` for any rule using the
            # ``headers`` predicate. The fetcher is cheap to build and
            # only invoked on cache miss inside ``_eval_headers``.

...
from m365ctl.onedrive.mutate.move import execute_move, finish_move, start_move
...
def run_move(
    *,
    ...
    if from_plan is not None:
        ...
        # Accept both bare ("move") and namespaced ("od.move") action
        # tokens — older plans use the bare form.
        ops = [op for op in plan.operations if op.action in ("move", "od.move")]
        ...
        def start_op(op, b, lg, *, before):
            return start_move(op, b, lg, before=befores.get(op.op_id, before))
        ...
        rc = execute_plan_in_batches(
            ...
            start_op=start_op, finish_op=finish_move,
            on_result=on_result,
        )

...
def run_copy(
    *,
    ...
    if from_plan is not None:
        ...
        ops = [op for op in plan.operations if op.action in ("copy", "od.copy")]
        ...
        def start_op(op, b, lg, *, before):
            return start_copy(op, b, lg, before=befores.get(op.op_id, before))

        def on_result(op, result):
            if result.status == "ok":
                print(f"[{op.op_id}] ok (monitor: "
                      f"{(result.after or {}).get('monitor_url', '?')})")
            else:
        ...
        rc = execute_plan_in_batches(
            ...
            start_op=start_op, finish_op=finish_copy,
            on_result=on_result,
        )

...
def _top_by_size_impl(conn, limit: int):
    from fazla_od.catalog.queries import top_by_size as q
    return q(conn, limit=limit)


def _stale_since_impl(conn, cutoff: str):
    from fazla_od.catalog.queries import stale_since as q
    return q(conn, cutoff=cutoff)


def _by_owner_impl(conn):
    from fazla_od.catalog.queries import by_owner as q
    return q(conn)


def _duplicates_impl(conn):
    from fazla_od.catalog.queries import duplicates as q
    return q(conn)

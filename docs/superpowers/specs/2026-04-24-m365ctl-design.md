...
- **Tenant:** target Microsoft 365 tenant (admin access held)
- **Next step:** implementation plan via `superpowers:writing-plans`

## 1. Goal

Give Claude Code CLI-driven control of a Microsoft 365 tenant's OneDrive + SharePoint content (tenant-wide) so that search, inventory, reorganization, cleanup, classification, content extraction, and sharing audits can be performed from a terminal without the M365 web UI.
...
- Single-tenant app registered in the target tenant: "m365ctl".
...
- **Primary auth: certificate.** Self-signed X.509, RSA-4096, SHA-256, 2-year validity.
  - Subject: `CN=m365ctl`
  - Private key: `~/.config/m365ctl/m365ctl.key` (mode 600, macOS FileVault at rest, never in git).
  - Public cert: `~/.config/m365ctl/m365ctl.cer` uploaded to Entra.
  - SHA-1 thumbprint: `<your-cert-thumbprint>`
...
### Token caching
- Delegated tokens cached via MSAL in macOS Keychain item `m365ctl:DelegatedTokenCache`.
...
cert_path    = "~/.config/m365ctl/m365ctl.key"   # PEM, private key
cert_public  = "~/.config/m365ctl/m365ctl.cer"   # PEM, public cert
...
Credentials (`~/.config/m365ctl/*`) live **outside** the repo and are never referenced by relative path.

## 10. Open items before implementation

The following must be resolved in the implementation-plan session:

1. **Label which UUID is which.** User provided two placeholder UUIDs; exact assignment to `tenant_id` vs `client_id` must be confirmed against the Entra app Overview page before either is written to `config.toml`.

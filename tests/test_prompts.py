...
from m365ctl.common.prompts import confirm_or_abort, TTYUnavailable


def test_confirm_returns_true_on_y(monkeypatch) -> None:
    fake_tty = io.StringIO("y\n")
    fake_out = io.StringIO()
    monkeypatch.setattr("m365ctl.common.prompts._open_tty", lambda: (fake_tty, fake_out))
...
def test_yes_flag_shortcuts_prompt(monkeypatch) -> None:
    ...
    monkeypatch.setattr("m365ctl.common.prompts._open_tty", should_not_open)
...
def test_raises_when_tty_unavailable(monkeypatch) -> None:
    ...
    monkeypatch.setattr("m365ctl.common.prompts._open_tty", no_tty)
